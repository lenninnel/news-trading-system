...
- **Bar-freshness gate (`scheduler/bar_freshness.py`, hook in `DailyScheduler._execute_run`)**: before a signal/execution session runs, every store ticker's `MAX(date)` must be the expected bar (T-1 intraday, T for EOD). Stale tickers are skipped, a fully stale store aborts the session; both log ERROR, alert via Telegram and write `session_runs.note` (additive column), which the watchdog reports (`ABORTED…` → session check fails). EOD waits up to `BAR_FRESHNESS_EOD_WAIT_S` for a late ingest. Fail-open on check crashes. Settings: `BAR_FRESHNESS_GATE_ENABLED`, `BAR_FRESHNESS_EOD_WAIT_S`, `BAR_FRESHNESS_POLL_S`.
- **Bar provenance on every signal row (`signal_events`)**: `indicator_bar_date`, `indicator_bar_close`, `indicator_bar_source`, `live_price`, `live_price_source` (additive, NULL on legacy rows). `TechnicalAgent` tags its daily frame with its source (`daily_ohlc` / `yfinance` / `eodhd` / `binance`) and exposes `bar_date`/`bar_close`/`bar_source` next to the indicators; the coordinator writes them on Combined and strategy rows. `price_at_signal` keeps its meaning (= indicator-bar close) and is now explained by the new columns.
- **Outcome tracker on the store (`analytics/outcome_tracker.py`)**: was NOT dead (20 EOD runs / 30 d, last fill 2026-09-22 22:20 UTC) but ran on a second price path (Alpaca bars, then yfinance). Now fills 3d/5d/10d from `daily_ohlc` only, same horizon semantics; new `outcome_status` / `outcome_note` / `outcome_source` / `outcome_updated_at`. Sentinel rows and tickers without store bars become `unevaluable` instead of silently pending (421 rows on prod); missing entry prices (scanner rows) are backfilled from the store's raw close. Always logs a summary; EOD crash is ERROR + Telegram. `scripts/update_outcomes.py` is a thin wrapper (yfinance fixups removed).
- **RiskAgent skips (`agents/risk_agent.py`)**: `event_risk_flag`, `days_to_earnings`, `regime` are now persisted on the skip path (3,089 prod rows had flag=none/NULL incl. all 20 real earnings skips); an unknown earnings date logs "earnings filter open". Thresholds unchanged.
- **Alpha Vantage key (`data/price_fallback.py`)**: reads `ALPHA_VANTAGE_KEY` or `ALPHA_VANTAGE_API_KEY`. Reported: the whole `PriceFallback` chain is not wired into the live path (`MarketData.fetch` = Alpaca trade → quote → degraded); left as is.
- Doc: `docs/DATA_PATHS_2026-09-23.md` (evidence, rules, VPS commands).

### 2026-09-08 — Re-entry lock after a stop-loss exit + holiday-aware stale guard (dev)

- **Re-entry lock (`execution/portfolio_manager.py`, gate 1b)**: the Q-016 1-session cool-down becomes a 5-session lock (`config/settings.py` `REENTRY_LOCK_SESSIONS`), counted in US trading sessions via `data/market_calendar.py` — the weekday-only counter it replaces treated Fri 2026-07-03 (holiday) as a session and let TRGP and VRT back in on the Monday (VRT −2 902 USD). Every rejected BUY is now also written to `signal_events` (`strategy='PortfolioManager'`, `signal='HOLD'`, `signal_path='REENTRY_LOCK'`, `bear_case` with the stop timestamp, sessions elapsed / remaining and the first eligible date, `session` and `price_at_signal` from the caller). All four entry paths in `orchestrator/coordinator.py` pass `session=`/`price=` into `can_add_position`; no path bypasses the gate. Trailing-stop exits arm the lock only with `REENTRY_LOCK_INCLUDE_TRAILING` (off). Duration derived from the audited gap distribution (62 % of after-stop re-entries within 4 sessions, sessions 5–6 empty), not from P&L — `docs/REENTRY_LOCK_2026-09-08.md`, `scripts/reentry_after_stop_audit.py` (read-only). The audit also shows the shadow book's −1.02 % selection effect is NOT carried by these re-entries (they are the better half of the filled signals in the shadow model).
- **Stale-feed guard (`monitoring/position_manager.py`)**: `_is_market_hours()` consults the NYSE holiday calendar, so a stale bar on Labor Day / Thanksgiving / Christmas is journal-only. Telegram cadence: one `STALE FEED` message at onset, one reminder every `_STALE_REALERT_MINUTES` (60) while stale, one `FEED RECOVERED` line on the first fresh bar (was: one message per 60 s poll — several hundred on 2026-09-07). Evaluability rule unchanged: no exit is evaluated on a stale price.
- **`monitoring/price_monitor.py`** (not a running service): same calendar for the US window, `US holiday` status, wake-up computed from the next trading-day open. XETRA window stays weekday-only (no EU calendar).
- **`data/market_calendar.py`**: `us_sessions_between`, `next_us_trading_day`, `is_us_rth`, `next_us_rth_open`.
- **Not changed, reported**: `scheduler/daily_runner.py` still fires all sessions on US holidays (US_PRE/EOD wrote 89 signal rows on 2026-09-07); `scripts/watchdog.py::_due_sessions`, `data/events_feed.py::_trading_days_between`, `api/main.py::_add_business_days` remain weekday-based — see doc §5.
