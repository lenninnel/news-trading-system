...
from execution.broker_factory import get_trading_mode
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Execute a paper trade based on the risk sizing result.",
    )
    ...
    _MODE_LABELS = {
        "paper_local": "LOCAL PAPER (simulated)",
        "alpaca_paper": "ALPACA PAPER (sandbox)",
    ...
    if args.agent == "sentiment":
        ...
        report = coordinator.run_combined(
            ticker, account_balance=args.balance, execute=args.execute,
        )
